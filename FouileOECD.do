/* programme de fouille des données issuers du programme de Céline */
...
/* 10/02/2014   Selection base restreinte  */

gen tap = (g7_norml_water ==1)
tabstat tap, by(country) sta(mean  N)

encode country, generate(Ncountry)

/* Variables Céline (new)*/
...
ren b3_concern_waste b03_lessconcernwaste

gen i_under18 = 0
replace i_under18 = 1 if a3_under18 >0

gen i_car = 0
replace i_car = 1 if d01_cars > 0

global yvars " g7_norml_water i_tap"
global endog1 "b2_concern_envir   b3_envconcern_index  b4_vote b5_envir  b06_env_attid_indx "  
global endog2 "b11_knowledge  b09_policy_indx b10_policy_indx  g8_wtrstfcn_health g8_wtrstfcn_taste"
global endog3 "b06_env_att2 b06_env_att6  isatis_health b03_lessconcernwaste b08_locenv_water "
global varendog  $endog1  $endog2 $endog3

global varindiv1 "i_male a2_age a3_hhsize i_posthighsch i_town a6_empl i_empl  i_owner  lowincome  log_income"
global varindiv2 "a11_residtype a12_residsize a13_residarea i_town  a14_residten unitcharge i_car i_under18 unitcharge"
global varindiv  $varindiv1  $varindiv2 
		
sum $varindiv $yvars $varendog 
*codebook country $varindiv $yvar $varendog 

/* export pour R */
preserve
keep $yvars $varendog $varindiv country Ncountry 
save $root/data/OECD.dta, replace
restore

/* modèle base */
probit tap i.Ncountry, nolog
lstat
estimates store Pmini

/* Modèle indiviuel  */
global indiv1 "i_male a2_age a3_hhsize i_posthighsch   i_empl log_income"
global indiv2 "i_owner i.a11_residtype i.a12_residsize i.a13_residarea   i.a14_residten"
global indivnotused "i.a6_empl  i_town"
global indiv  $indiv1  $indiv2 

probit tap  i.Ncountry $indiv, nolog
estimates store Pindiv
lstat   

est table Pmini Pindiv , stats(N ll chi2 df_m aic r2_p ) star style(noline) b(%7.3f) 

/* Probits after Np estimation on all endog and indiv variables...  */

probit tap i.Ncountry $indiv ///
			b3_envconcern_index  i.b06_env_att6 b10_policy_indx ///
			g8_wtrstfcn_health 	g8_wtrstfcn_taste
			
estimates store PindivNP  /* On gagne un peu en prédiction, mais pas en significativité */
fitstat, saving(Pnpi)
lstat

/* modèle plus parcimonieux issu de l'estimation NP sous  R */

probit tap i.Ncountry  a2_age i_empl i_owner  unitcharge /*i_town*/ i.a12_residsize i.a13_residarea   ///
				/*i.b2_concern_envir*/ b3_envconcern_index i.b06_env_att6 b10_policy_indx /*b11_knowledge*/  ///
				g8_wtrstfcn_health g8_wtrstfcn_taste  
			 
lstat	
estimates store Pnp
lstat		/* Confusion matrix  */
margeff   /* effets marginaux  */

est table Pmini Pindiv* Pnp , stats(N ll chi2 df_m aic r2_p ) star style(noline) b(%7.3f) label

fitstat, using(Pnpi) force  /* Modele Pnp préféré  !!*/

probit tap i.Ncountry  a2_age i_empl i_owner lowincome unitcharge i_town i.a12_residsize i.a13_residarea    ///
				i.b2_concern_envir b3_envconcern_index i.b06_env_att6 b10_policy_indx /*b11_knowledge*/  ///
				g8_wtrstfcn_health g8_wtrstfcn_taste  
estimates store PnpNew

lstat		/* Confusion matrix  */
margeff   /* effets marginaux  */


est table Pmini Pnp PnpNew , stats(N ll chi2 df_m aic r2_p ) star style(noline) b(%7.3f) label
 /* test de modèles */


/* quelques stats des  */
graph hbox _index , nooutsides over(tap) over(country) asyvars missing

predict resid, stdp  
label var resid "Résidus standardisés (pearson)"
predict cook, dbeta
hist resid, kdens

/* Modèle Céline (12/02) */

probit i_tap   a2_age i_under18 i_posthighsch lowincome  i_town i_car  isatis_health    ///
				b08_locenv_water b3_envconcern_index b03_lessconcernwaste /* b7_trust_govt */  ///
				i.Ncountry
estimates store PCeline

est table PCeline , stats(N ll chi2 df_m aic r2_p ) star style(noline) b(%7.3f) label
