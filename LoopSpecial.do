...
log using "Logs\ResultsTrim`TrimE'", replace
...
quietly Monsspecialreg2 i_tap Special, exog($exog) endog($endog) iv($instrument) hetero trim(`t') 
...
graph export "Graphs/CoefsTrim`TrimE'.pdf", replace
translate "Logs/ResultsTrim`TrimE'.smcl"  "Logs/ResultsTrim`TrimE'.pdf", translator(smcl2pdf)  pagesize(a4) ///
          cmdnumber(off) logo(off) fontsize(8) header(off)  replace

/* Récupération des effets marginaux   */
